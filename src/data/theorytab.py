...
class Theorytab:
  ...
  def _extract_tonic(self, meta_element):
    ...
    if key_element.string not in self.NOTE_TO_PITCH_CLASS:
      logger.error("%s has unrecognized key %s", self.filename, key_element.string)
      return None
  ...
  def _extract_youtube_id(self, meta_element):
    ...
    if not self.YOUTUBE_ID_REGEX.match(youtube_element.string):
      logger.error("%s has an invalid YouTube id %s", self.filename, youtube_element.string)
      return None
  ...
  def _extract_timing(self, meta_section):
    ...
    if (None in (global_start, active_start, active_stop) or
        None in (global_start.string, active_start.string, active_stop.string)):
  ...
  def clips(self):
    ...
    if meta_element.find('sections', recursive=False):
      meta_sections = [elem for elem in meta_element.find('sections', recursive=False).contents
                       if type(elem) != bs4.element.NavigableString]
      data_sections = [elem for elem in root_element.find('sections', recursive=False).contents
                       if type(elem) != bs4.element.NavigableString]
    else:
    ...
    for meta_section, data_section in zip(meta_sections, data_sections):
      if not meta_section or not data_section:
        ...
        continue
      if meta_section.name != 'meta' and data_section.name != 'data' and meta_section.name != data_section.name:
        logger.warning("%s section name mismatch: %s and %s", self.filename, meta_section.name, data_section.name)
